...
public class Mac extends MacSkeleton {
     ...
     */
    public byte[] mac(byte[] message, Key key) throws Exception {
        int n = message.length, blocksize = getBlockSize(), blocks = n / blocksize;
        boolean partial;
        byte[] tag;
        ...
        if (partial) { // leftover data, need to add another block to fill
            blocks++;
        }

        message = pad(message, blocks * blocksize); // resizing
        tag = encryptBlock(Arrays.copyOfRange(message, 0, blocksize), key); // initial block = no XORing

        for (int i = 1; i < blocks; i++) {  // subsequent blocks feed off of initial's output
            byte[] nextBlock = Arrays.copyOfRange(message, 0, blocksize);
            tag = encryptBlock(xor(tag, nextBlock), key);
        }
     ...
     */
    public boolean verify(byte[] message, byte[] tag, Key key) throws Exception {
        if (Main.toHexString(tag).equals(Main.toHexString(mac(message, key)))) {
            return true;
        }
        return false;
    }
